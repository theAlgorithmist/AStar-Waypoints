...
export class AstarMinHeap
{
  ...
  public toArray(): Array<AStarWaypoint>
  {
    ...
    for (i = 1; i <= this._position; ++i) {
      waypoints.push(this._data[i] as AStarWaypoint);
    }
  ...
  public extractRoot(): AStarWaypoint | null
  {
    ...
    let root: AStarWaypoint = this._data[1];
  ...
  /**
   * Clear the heap and prepare for new data; the heap is cleared and prepared for new data, however, the current heap
  ...
  protected __heapifyDn(k: number): void
  {
    ...
    {
      let tmp: AStarWaypoint = this._data[minIndex];
